...
public class SslClient : IDisposable
{
    ...
    {
        if (IsConnected || IsHandshaked || IsConnecting || IsHandshaking) return false;
        ...
        // Update the handshake flag
        IsHandshaked = true;

        // Call the session handshake handler
        OnHandshaked();
    ...
    public virtual bool Reconnect() => Disconnect() && Connect();
    ...
    /// </summary>
    public virtual void ReceiveAsync() => TryReceive(); // Try to receive data from the server
    ...
    private void TryReceive()
    {
        ...
        try
        {
            // Async receive with the handler
            IAsyncResult result;
            do
            {
                if (!IsHandshaked)
